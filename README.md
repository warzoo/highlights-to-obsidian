...
- You can group a book's highlights by chapter. Enable "Group highlights by first-level chapter, with a clickable table of contents" in the config's Formatting Options. Each book then becomes a single note where highlights are placed under `## chapter` headings (the book's first-level table-of-contents sections), with a list of `[[#chapter]]` links at the top that jump to each section. Set the "Table-of-contents heading" to whatever label you want above those links, e.g. `## Contents` or `## 目录` (leave it empty to skip the link list). This works best with "Write highlights directly to vault files" enabled, since the in-note links only resolve when the whole book is in one file — so in this mode the note is never split by the max note size. Sort by `location` to keep chapters in reading order. It isn't combined with "Keep notes sorted and preserve your edits" (grouping takes over if both are on), and like the header, re-sending appends a fresh table of contents, so it's best used when sending a book's highlights together.
...
- {topchapter}: Like {chaptertitle}, but the first-level (broadest) table-of-contents section instead of the deepest, e.g. "Chapter 1" or "Part I". This is the level the "Group highlights by first-level chapter" option (see below) buckets highlights by.
